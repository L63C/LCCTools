...
@objcMembers public class LocalLog: NSObject {
    ...
    public static func save(content: String, maxSize:Int = 5, directory: String = NSHomeDirectory(), fileName: String = "default.txt"){
        ...
        queue.async {
            ...
            let writeInfo = self.currentDataStr().appending(" ").appending(content).appending("\n")
    ...
    static func write(toPath path: String,content: String) {
        ...
        fileHandle?.write(content.data(using: .utf8) ?? Data.init())
        fileHandle?.closeFile()
    }
    static func currentDataStr() -> String {
  ...
  public static func deleteHalfContent(path:String,maxSize:Int) -> String{
        do {
            ...
            if fileSize > maxSize * 1024 * 1024{
                ...
                let tempPath = self.createFile(directory: directory!, fileName: "temp_"+fileName)
